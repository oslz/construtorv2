...
client.on('ready', () =>{
    setInterval(() =>{
        client.user.setPresence({ game: { name: 'minha vida', type: 1, url: 'https://www.twitch.tv/olszera'} });
        client.user.setPresence({ game: { name: 'aids', type: 1, url: 'https://www.twitch.tv/olszera'} });
        client.user.setPresence({ game: { name: 'problemas', type: 1, url: 'https://www.twitch.tv/olszera'} });
        client.user.setPresence({ game: { name: `${client.guilds.size} guild diferentes`, type: 1, url: 'https://www.twitch.tv/olszera'} });
        },1000 * 30)
...
try {
let commandFile = require(`./construtor/${command}.js`);
...
client.on('message', async msg => { // eslint-disable-line
	...
	if(command == "comandos") {
		...
		.setAuthor("Construtor DJ", client.user.avatarURL)
        .setColor('#365be2')
        .addField('- Principal:', '**Prefix**: `!`')
		.addField("- !comandos", "Mostra esta lista de comandos.")
...
Escolha uma musica de 1-10.
		...
		serverQueue.connection.dispatcher.end('Skip por ' + msg.author.username);
		return undefined;
